...
const root = path.join(__dirname, "..", "themes", "packs");

function ensure(dir) {
  fs.mkdirSync(dir, { recursive: true });
}

function previewSvg(bg1, bg2, panel, btn1, btn2, accent, label, ink = "#f3efe6") {
...
function themeCss(id, vars, bodyExtra, panelExtra, btnExtra, extras = "") {
...
function writePack(id, previewArgs, cssArgs) {
  const dir = path.join(root, id);
  ensure(dir);
  fs.writeFileSync(path.join(dir, "preview.svg"), previewSvg(...previewArgs));
  if (cssArgs) fs.writeFileSync(path.join(dir, "theme.css"), themeCss(id, ...cssArgs));
}

ensure(root);

writePack("classic", ["#121212", "#070707", "#1a1a1a", "#f0d78c", "#d4af37", "#d4af37", "CLASSIC"]);
...
console.log("OK", fs.readdirSync(root).join(", "));
